...
/**
 * Partitions incoming events, and generates directories and file names in which to store the incoming events.
 */
public abstract class Partitioner extends Configured {

    /**
     * Encode partition values into a string, to be embedded into the working filename. Encoded values cannot use '/' or
     * ':'. Use partition values in the etlKey. Values should be extracted from the Record and given to the
     * CamusWrapper. The return of the method will be passed as the "encodedPartition" parameter of
    ...
    /**
     * Return a string representing the partitioned directory structure where the .avro files will be moved. For
     * example, if you were using Hive style partitioning, a timestamp based partitioning scheme would return
     * topic-name/year=2012/month=02/day=04/hour=12 The return of this method will be prepended with the value of the
     * property etl.destination.path Most users will want to start this path with the topic name.
     * 
     * @param context The JobContext
     ...
     * @return A path string where the avro files will be moved to.
     */
    public abstract String getWorkingFileName(JobContext context, String topic, String brokerId, int partitionId,
