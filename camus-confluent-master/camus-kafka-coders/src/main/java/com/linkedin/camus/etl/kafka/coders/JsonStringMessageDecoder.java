...
/**
 * MessageDecoder class that will convert the payload into a JSON object, look for a the camus.message.timestamp.field,
 * convert that timestamp to a unix epoch long using camus.message.timestamp.format, and then set the CamusWrapper's
 * timestamp property to the record's timestamp. If the JSON does not have a timestamp or if the timestamp could not be
 * parsed properly, then System.currentTimeMillis() will be used.
 * <p/>
 * camus.message.timestamp.format will be used with SimpleDateFormat. If your camus.message.timestamp.field is stored in
 * JSON as a unix epoch timestamp, you should set camus.message.timestamp.format to 'unix_seconds' (if your timestamp
 * units are seconds) or 'unix_milliseconds' (if your timestamp units are milliseconds).
 * <p/>
 * This MessageDecoder returns a CamusWrapper that works with Strings payloads, since JSON data is always a String.
 */
